...
def select_initial_step(fun, t0, y0, f0, p_scatter0, direction, order, rtol, atol):
    ...
    if p_scatter0 > 0:
        return min(100 * h0, h1, 1/p_scatter0)
...
class OdeSolution(object):
    ...
    def __init__(self, ts, interpolants):
        ...
        if not ((ts.size == 2 and ts[0] == ts[-1]) or np.all(d > 0) or np.all(d < 0)): raise ValueError("`ts` must be strictly increasing or decreasing.")
