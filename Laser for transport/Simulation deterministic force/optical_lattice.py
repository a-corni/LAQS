...
def E_pol(z,y,x):
    ...
    beam1 = E1(z,y,x)*np.exp(-1j*k1*z)
    pol2 = np.array([1j/np.sqrt(2),1/np.sqrt(2),0])
    beam2 = E2(x,z,y)*np.exp(-1j*k2*x)
    return (beam1 + np.conjugate(beam1))*pol1 + (beam2 + np.conjugate(beam2))*pol2
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def animate(self):
        ...
        fname = 'D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/optical_lattice/frame_nb_'+str(self.counter)+'.png'
        image.save(fname)
        self.fbo.release()
