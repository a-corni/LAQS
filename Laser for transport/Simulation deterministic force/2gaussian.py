...
dt = 0.01*(1.0/wx)/tL # 5% period in tL unit
...
def E_pol(t, z, y, x):
    ...
    beam1 = E1(z,y,x)*np.exp(-1j*k1*z+1j*wt1*t)
    beam2 = E2(t,z,y,x)*np.exp(1j*k2*z+1j*wt2(t)*t)
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def __init__(self, parent=None,smooth=True):
        ...
        data = np.load('D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/solverp0_3D_2gaussian.npz',allow_pickle=True)
        ...
        data = np.array([[It0(0, (zi+z0/L)*L, xi*L, 0.0) for zi in zarr] for xi in xarr]) #stores the information about the intensity profile
        ...
        plt.imsave('D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/2gaussian/tmp'+str(self.counter)+'.png', image)
        
        #something misterious
        i = 0
        self.texture_id.append(i)
        
        img = Image.open('D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/2gaussian/tmp'+str(self.counter)+'.png').convert("RGBA")
        itm = np.asarray(img)
        itm2 = itm.copy()
        #itm2[:,:,0] = data[:,:]
        self.pixels=itm2[:]
