...
dt = 0.01*(1.0/wx)/tL # 1% period in tL unit
...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def __init__(self, parent=None,smooth=True):
        ...
        data = np.array([[It0(self.counter*dt*tL,(zi+z0/L)*L,xi*L,0.0) for zi in zarr] for xi in xarr]) #stores the information about the intensity profile
        ...
        d.text((0,0), str(self.counter), fill=(255,255,255,255))
        itm = np.asarray(img)
        itm2 = itm.copy()
        #itm2[:,:,0] = data[:,:]
