...
#simulation to run
simulation_name = "doughnut_0.5_10"
...
for i,vi in enumerate(vzr):
    ...
    sol = solve_ivp(af1, (tstart1,tend1), (vzi,zi,vyi,yi,vxi,xi), t_eval = np.linspace(tstart1,tend1,int((tend1-tstart1)/dt),endpoint=True),method=BESTMETHOD,atol=1e-6,rtol=1e-10)#,atol=1e-6,rtol=1e-10)#, t_eval=None, dense_output=False, events=None, vectorized=False, **options)[source]
...
for i,vi in enumerate(vzr):
    ...
    sol = solve_ivp(af2, (tstart2,tend2), (vzi,zi,vyi,yi,vxi,xi), t_eval = np.linspace(tstart2,tend2,int((tend2-tstart2)/dt),endpoint=True),method=BESTMETHOD,atol=1e-6,rtol=1e-10)#,atol=1e-6,rtol=1e-10)#, t_eval=None, dense_output=False, events=None, vectorized=False, **options)[source]
