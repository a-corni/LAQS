...
def af2(t,y):
    ...
    # red detuned
    return [-g-derivTz(pz*L,py*L,px*L)*L,vz,-derivTy(pz*L,py*L,px*L)*L,vy,-derivTx(pz*L,py*L,px*L)*L,vx]
...
for i,vi in enumerate(vzr):
    ...
    zi = np.random.normal(0.0,1e-6)/L
    yi = np.random.normal(0.0,1e-6)/L
    xi = np.random.normal(0.0,1e-6)/L
...
nb_part = len(sol_arr)
nb_iter = sol.y.shape[1]
print('number of simulated particules :', nb_part)
print('number of iterations', nb_iter)

print('--saving traj data to .npz file--')
np.savez('D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/solverp0_3D_doughnut.npz',sol_arr=sol_arr)
print('--done--')
