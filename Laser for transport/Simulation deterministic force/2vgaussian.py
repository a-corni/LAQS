...
la = 1.e-6 #beam wavelength 
w0 = 5e-6 #beam waist
k = 2.0*np.pi/la #wave vector

zR = np.pi*w0**2/la #Rayleigh length
L = w0/np.sqrt(2.0) #caracteristical length along z

vL = np.sqrt(2.0*U0/mass) #caracteristical speed length
tL = L/vL #caracteristical time
z0 = 0.0 #maximum intensity of the beam
...
def E_pol(z,y,x):
    ...
    beam1 = E(z,y,x)*np.exp(-1j*k*z)*pol1
    pol2 = np.array([1j/np.sqrt(2),1/np.sqrt(2),0])
    beam2 = E(x,z,y)*np.exp(-1j*k*x)*pol2
    return beam1 + np.conjugate(beam1) + beam2 + np.conjugate(beam2)
...
# Constructs a QGLFormat object with the following default settings:
    ...
    # Multisample buffers: Disabled
        ...
        data = np.load('D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/solverp0_3D_optical_lattice.npz',allow_pickle=True)
