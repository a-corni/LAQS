...
def It(z,y,x):
    r2 = x**2+y**2
    return (w0/w(z))**2*np.exp(-2*r2/w(z)**2)

    
def derivTx(z,y,x):
    return -4*x*It(z,y,x)/w(z)**2
    
def derivTy(z,y,x):
    return -4*y*It(z,y,x)/w(z)**2
    
def derivTz(z,y,x):
    r2 = x**2+y**2
    ret = -2*(z-z0)/zR**2/(1+(z-z0)**2/zR**2)**2*np.exp(-2*r2/w(z)**2)+It(z,y,x)*4*r2*(z-z0)/w0**2/zR**2/(1+(z-z0)**2/zR**2)**2
    return ret
...
np.savez('solverp0_3D_simple.npz',sol_arr=sol_arr)
print('--done--')
