...
z0 = np.sqrt(5)*zR #position of maximum intensity along the z-axis
...
def E_pol(z,y,x):
    ...
    beam1 = E(z-z0,y,x)*np.exp(-1j*k*(z-z0))*pol
    beam2 = E(z+z0,y,x)*np.exp(-1j*k*(z+z0))*pol
    return beam1 + np.conjugate(beam2)
...
np.savez('D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/solverp0_3D_2gaussian_shifted.npz',sol_arr=sol_arr)
print('--done--')
