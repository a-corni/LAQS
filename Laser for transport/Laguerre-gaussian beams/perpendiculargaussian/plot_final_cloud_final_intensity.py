...
simulation_name = "perpendicular_gaussian_transport"
...
Z = [-0.1+2e-3*2*(k-5000)/10000 for k in range(10000)]
plt.plot(Z,[It(z,0,0,time[-1]) for z in Z]) 
pz = [sol_arr[j].y[1,-1]*L for j in range(nb_part)]
py = [0.5+sol_arr[j].y[3,-1]*L for j in range(nb_part)]
plt.scatter(pz,py, s = 0.1)
plt.title("Position and intensity of the beam at time "+'%.3f'%(time[-1])+" s")
plt.xlabel("z axis (m)")
plt.ylabel("Intensity along z, at time t = "+"%.3f"%(time[-1])+" s")
plt.savefig("D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/perpendiculargaussian/intensity_it_d_0_1"+str(-1))
