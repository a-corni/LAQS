...
simulation_name = "2gaussian_50_ms"
...
yp = np.array([-w0+2*w0*k/100 for k in range(100)])*1e3 #np.array([la1*2*(k-50)/10 for k in range(100)])*1e6
#mean_z = sol_arr[0].y[1,-1]*L+sol_arr[4].y[1,-1]*L+sol_arr[6].y[1,-1]*L+sol_arr[8].y[1,-1]*L+sol_arr[11].y[1,-1]*L+sol_arr[13].y[1,-1]*L+sol_arr[14].y[1,-1]*L+sol_arr[18].y[1,-1]*L
mean_z = sol_arr[0].y[1,-1]*L
#mean_z = mean_z
print(mean_z)
zp = np.array([mean_z+la1*2*(k-500)/1 for k in range(1000)])*1e3
print(zp[500])
print(yp[50])
print(yp[0])
Itensity = np.zeros((1000,100))

for i,y in enumerate(yp):
    for j,z in enumerate(zp):
            Itensity[j,i] = It(zp[j]*1e-3, yp[i]*1e-3, 0, time[-1]*tL)

plt.xlabel('y (mm)') 
plt.ylabel('z+'+'%.3f'%(mean_z)+' (mm)') 
plt.xlim([-w0*1e3,w0*1e3])
plt.ylim([(mean_z-1000*la1)*1e3, (mean_z+la1*1000)*1e3])
print("time = ", time[-1]*tL) 
plt.title("Intensity at time "+'%.3f'%(time[-1]*tL*1e3)+"ms") 
...
plt.tight_layout()
pz = np.array([sol_arr[j].y[1,-1]*L for j in range(nb_part)])*1e3
print("pz = ", pz)
py = np.array([sol_arr[j].y[3,-1]*L for j in range(nb_part)])*1e3
print("py = ", py)
plt.scatter(py,pz,s = 1.0, c = 'r')
plt.savefig('D:/Users/Antoine/Documents/copenhague-1/togit/gaussian_forAntoine/2gaussian/intensity_it'+str(-1)+'_50ms.png')
plt.close()
