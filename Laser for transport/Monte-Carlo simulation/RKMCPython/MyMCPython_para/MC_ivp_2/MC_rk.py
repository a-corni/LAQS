import numpy as np
from .MC_base import OdeSolver, DenseOutput
...
class RungeKutta(OdeSolver):
    ...
    def __init__(self, fun, fun_scattering, proba_scattering, t0, y0, t_bound, max_step=np.inf,rtol=1e-3, atol=1e-6, vectorized=False,first_step=None, **extraneous):
        ...
        p_scatter = self.proba_scattering(1.0, y0)
        if first_step is None:
            self.h_abs = select_initial_step(self.fun, self.t, self.y, self.f, p_scatter, self.direction, self.error_estimator_order, self.rtol, self.atol)
        else:
            self.h_abs = validate_first_step(first_step, t0, t_bound)
        self.p_scatter = self.proba_scattering(self.h_abs, y0)
            ...
            if error_norm < 1:
                ...
                if p_scatter < 0.2 :
                    ...
                    if scatter_event :
                        ...
                        y_new = self.fun_scattering(t_new, y_new) 
